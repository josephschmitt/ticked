...
export default function SettingsFieldMappingScreen() {
  ...
  // Handle save - dismiss modal when done
  const handleSave = useCallback(async () => {
    ...
    // Dismiss the entire settings modal
    router.dismiss();
  }, [canSave, mapping, setFieldMapping, router]);
    ...
    <SafeAreaView className="flex-1 bg-background-grouped dark:bg-background-dark-grouped" edges={["bottom"]}>
      ...
      <View className="px-4 py-4 border-t border-separator dark:border-separator-dark bg-background-elevated dark:bg-background-dark-elevated">
        <Pressable
          ...
          style={{
            backgroundColor: canSave ? BRAND_COLORS.primary : (isDark ? '#3A3A3C' : '#D1D1D6'),
          }}
        >
          <Text
            className={`text-[17px] font-semibold ${
              canSave ? "text-white" : "text-label-tertiary dark:text-label-dark-tertiary"
            }`}
          >
            Save Changes
          </Text>
